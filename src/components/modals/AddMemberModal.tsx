...
export default function AddMemberModal({ isOpen, onClose }: AddMemberModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg("Family member saved successfully to PostgreSQL Database!");
      setTimeout(() => {
        setFirstName("");
        setLastName("");
        setBio("");
      ...
      <div className="bg-white dark:bg-stone-900 border border-slate-200 dark:border-stone-800 rounded-3xl w-full max-w-lg shadow-2xl p-6 space-y-6 relative overflow-hidden">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium">
                Save a new relative to your PostgreSQL sanctuary database.
              </p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          ...
          <div>
            ...
            <textarea
              rows={3}
              placeholder="Add biography, achievements, or relationship notes..."
              value={bio}
